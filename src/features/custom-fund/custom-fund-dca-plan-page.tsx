...
import { readCustomFundDraftCache, writeCustomFundDraftCache, type CustomFundDraftCache } from "./custom-fund-draft-store";
...
export function CustomFundDcaPlanPage({ marketId, language: languageProp = "en" }: { marketId: MarketId; language?: Language }) {
  const language = useResolvedLanguage(languageProp);
  const location = useLocation();
  const navigate = useNavigate();
  const [cache] = useState<CustomFundDraftCache | null>(() => readCustomFundDraftCache(marketId));
  ...
  function applyPlans() {
    if (!cache) returnToCustomFund();
    if (!cache) return;
    writeCustomFundDraftCache({
      marketId: cache.marketId,
      language,
      editingId: cache.editingId,
    ...
    return (
      <div className="space-y-5">
        <SecondaryButton onClick={returnToCustomFund}>
          <span className="inline-flex items-center gap-2"><ArrowLeft size={16} /> {t(language, "custom.returnToCustomFund")}</span>
        </SecondaryButton>
        <StatusBanner title={t(language, "custom.noDraftForDca")} body={t(language, "custom.noDraftForDcaBody")} />
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-5 border-b border-zinc-200 pb-5 dark:border-white/10">
        ...
        <div className="mb-2 text-xs font-medium uppercase tracking-wide text-emerald-600 dark:text-emerald-400">{t(language, "custom.dcaPlanTitle")}</div>
        <h1 className="text-3xl font-semibold tracking-tight text-zinc-950 dark:text-white sm:text-4xl">{cache.draft.name}</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-500 dark:text-zinc-400 sm:text-base">{t(language, "custom.dcaPlanSubtitle")}</p>
      </div>
      <WorkbenchLayout
        align="start"
        pool={
          <WorkbenchPanel title={t(language, "portfolio.planSnapshot")} subtitle={t(language, "custom.dcaPlanSnapshotSubtitle")} className="flex flex-col lg:min-h-[38rem] xl:h-[40rem]">
            <div className="grid grid-cols-2 gap-x-5 gap-y-4 border-b border-zinc-100 pb-4 dark:border-white/10">
              <PreviewMetric label={t(language, "custom.selectedAssets")} value={formatNumber(assets.length)} />
        ...
        controls={
          <WorkbenchPanel
            ...
            subtitle={t(language, "custom.dcaPlanDetailsSubtitle")}
