...
import type { AssetRecord, Exposure, Holding, MarketId, Metric, PortfolioDcaPlan, PortfolioSummary } from "@/lib/types";
...
import { readPortfolioResultCache, type PortfolioResultCache } from "./portfolio-result-store";
...
export function PortfolioResultPage({ marketId, language = "en" }: { marketId: MarketId; language?: Language }) {
  const location = useLocation();
  const navigate = useNavigate();
  const portfolio = usePortfolio(marketId);
  const [result] = useState<PortfolioResultCache | null>(() => readPortfolioResultCache(marketId));
  ...
  const portfolioId = result?.activePortfolioId ?? portfolio.data?.portfolio?.id ?? null;
  const metrics = useMemo(() => (summary ? buildMetrics(summary, marketId, language) : []), [language, marketId, summary]);
  const allocationSlices = useMemo(() => (summary ? buildHoldingAllocationSlices(summary.holdings, language) : []), [language, summary]);
  ...
  async function saveCalculatedHoldings() {
    if (!result) {
      setStatus(t(language, "portfolio.createBeforeSave"));
      return;
    }
    setSaving(true);
    setStatus(t(language, "portfolio.savingHolding", { symbol: "funds" }));
    try {
      let targetPortfolioId = portfolioId;
      if (!targetPortfolioId) {
        const created = await portfolio.createPortfolio({
          name: result.input.name,
          goal: result.input.goal,
          riskPreference: result.input.riskPreference,
          ...
          valueHistory: result.result.summary.valueHistory,
          contributionHistory: result.result.portfolio.contributionHistory ?? [],
        });
        targetPortfolioId = created.id;
      }
      await portfolio.updatePortfolio(targetPortfolioId, {
        name: result.input.name,
        goal: result.input.goal,
        riskPreference: result.input.riskPreference,
        cashBalance: result.input.cashBalance,
      ...
      for (const holding of result.result.summary.holdings) {
      ...
      setSaved(true);
      setStatus(t(language, "portfolio.holdingSaved", { symbol: "funds" }));
    ...
    <div className="space-y-5">
      <div className="border-b border-zinc-200 pb-6 dark:border-white/10">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          {result ? (
            <button
              ...
              onClick={saveCalculatedHoldings}
            ...
            >
              <Save size={16} />
              {saved ? t(language, "dca.savedAction") : t(language, "portfolio.saveHolding")}
            </button>
          ) : null}
        </div>
        <div className="max-w-3xl">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-zinc-950 dark:text-white sm:text-4xl">{result?.input.name || t(language, "portfolio.resultTitle")}</h1>
          <p className="mt-3 text-sm leading-6 text-zinc-500 dark:text-zinc-400 sm:text-base">
            {result ? t(language, "portfolio.resultSubtitle") : t(language, "portfolio.noCachedResult")}
          </p>
        </div>
      </div>
      ...
      {!result || !summary ? (
        <StatusBanner
          ...
          body={t(language, "portfolio.noCachedResult")}
          action={<SecondaryButton onClick={returnToPortfolio}>{t(language, "portfolio.returnToLab")}</SecondaryButton>}
        />
      ) : (
        <>
            ...
            <div className="min-w-0">
              <section className="h-full rounded-lg border border-zinc-200 bg-white p-3 dark:border-white/10 dark:bg-white/[0.03]">
                <div className="mb-3 flex min-w-0 items-center justify-between gap-3">
                  ...
                  >
                    <List size={14} />
                    {t(language, "portfolio.viewFunds")}
                  </button>
                </div>
                <div className="space-y-2 text-sm">
                  <SnapshotRow label={t(language, "portfolio.selectedFunds")} value={formatNumber(result.selectedAssets.length)} />
                  <SnapshotRow label={t(language, "common.targetWeight")} value={formatWeight(summary.holdings.reduce((total, holding) => total + holding.targetWeight, 0))} />
                  ...
                  <SnapshotRow label={t(language, "common.risk")} value={riskLabel(language, result.input.riskPreference)} />
                </div>
                {status ? (
          ...
          <WorkbenchPanel title={t(language, "portfolio.valueCurve")} subtitle={t(language, "portfolio.resultSubtitle")} className="overflow-hidden">
          ...
          <div className="grid items-stretch gap-5 lg:grid-cols-2">
            <PortfolioPie
              title={t(language, "portfolio.fundAllocation")}
              subtitle={t(language, "portfolio.allocationResultSource")}
              data={allocationSlices}
              centerLabel={t(language, "portfolio.selectedFunds")}
              centerValue={formatNumber(summary.holdings.length)}
              emptyTitle={t(language, "portfolio.selectFundEmpty")}
            />
            <PortfolioPie
          ...
          <Section title={t(language, "portfolio.holdings")} subtitle={t(language, "portfolio.resultSubtitle")}>
            <HoldingsTable holdings={summary.holdings} marketId={marketId} language={language} />
          </Section>
          {showFundList ? (
            <FundListModal assets={result.selectedAssets} language={language} onClose={() => setShowFundList(false)} />
          ) : null}
          {showDcaPlanDetails ? (
            <DcaPlanDetailsModal rows={dcaPlanRows} marketId={marketId} language={language} onClose={() => setShowDcaPlanDetails(false)} />
          ) : null}
        </>
      )}
    </div>
...
function buildMetrics(summary: PortfolioSummary, marketId: MarketId, language: Language): Metric[] {
  return [
    ...
    { label: t(language, "dca.maxDrawdown"), value: formatPercent(summary.maxDrawdown), tone: "negative" },
    { label: t(language, "common.riskScore"), value: formatNumber(summary.riskScore, 1) },
    { label: t(language, "portfolio.exposure"), value: summary.sectorExposure[0] ? localizedAssetSector(summary.sectorExposure[0].name, language) : t(language, "asset.other"), delta: summary.sectorExposure[0] ? formatWeight(summary.sectorExposure[0].weight) : undefined },
    { label: t(language, "portfolio.selectedFunds"), value: formatNumber(summary.holdings.length) },
  ];
}
...
function FundListModal({ assets, language, onClose }: { assets: AssetRecord[]; language: Language; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/35 px-4 py-8 dark:bg-black/60" role="dialog" aria-modal="true">
      ...
      <div className="relative flex max-h-[82vh] w-full max-w-2xl flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-xl dark:border-white/10 dark:bg-[#080d0c] dark:shadow-black/40">
        <div className="flex items-center justify-between gap-4 border-b border-zinc-100 px-4 py-3 dark:border-white/10">
          <div className="min-w-0">
            <h3 className="text-base font-semibold tracking-tight text-zinc-950 dark:text-white">{t(language, "portfolio.selectedFundList")}</h3>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{t(language, "portfolio.selectedFundCount", { count: formatNumber(assets.length) })}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            {assets.map((asset) => (
              <div key={asset.id} className="flex min-w-0 items-center justify-between gap-4 rounded border border-zinc-200 bg-zinc-50/70 px-3 py-2.5 dark:border-white/10 dark:bg-white/[0.04]">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 truncate text-xs text-zinc-500 dark:text-zinc-400">{asset.name}</div>
                </div>
    ...
    <div className="mt-4 border-t border-white/80 pt-4 dark:border-white/10">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate text-xs text-zinc-500 dark:text-zinc-400">{t(language, "portfolio.dcaPlanResultSubtitle")}</div>
        </div>
      ...
      <div className="relative flex max-h-[82vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-xl dark:border-white/10 dark:bg-[#080d0c] dark:shadow-black/40">
        <div className="flex items-center justify-between gap-4 border-b border-zinc-100 px-4 py-3 dark:border-white/10">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{t(language, "portfolio.dcaPlanResultSubtitle")}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
                ...
                <section key={row.asset.id} className="rounded border border-emerald-100 bg-emerald-50/45 p-3 dark:border-emerald-400/30 dark:bg-emerald-400/10">
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 truncate text-xs text-zinc-500 dark:text-zinc-400">{row.asset.name}</div>
                    </div>
...
function HoldingsTable({ holdings, marketId, language }: { holdings: PortfolioSummary["holdings"]; marketId: MarketId; language: Language }) {
  return (
    <div className="overflow-x-auto rounded-lg border border-zinc-200 bg-white dark:border-white/10 dark:bg-white/[0.03]">
      <table className="w-full min-w-[760px] text-sm">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-white/10">
          {holdings.map((holding: Holding & { marketValue: number; gainPercent: number; currentWeight: number; targetGap: number }) => (
            <tr key={holding.id}>
              <td className="px-4 py-3">
                <div className="font-medium text-zinc-950 dark:text-white">{holding.name}</div>
                <div className="text-xs text-zinc-500 dark:text-zinc-400">{holding.symbol} · {localizedAssetSector(holding.sector, language)}</div>
              </td>
              <td className="px-4 py-3 text-right font-medium">{formatCurrency(holding.marketValue, marketId)}</td>
              <td className="px-4 py-3 text-right"><ToneText tone={holding.gainPercent >= 0 ? "positive" : "negative"} marketTone>{formatPercent(holding.gainPercent)}</ToneText></td>
...
function buildHoldingAllocationSlices(holdings: PortfolioSummary["holdings"], language: Language): PieSlice[] {
...
function riskLabel(language: Language, value: string) {
  if (value === "Conservative") return t(language, "portfolio.riskConservative");
  if (value === "Growth") return t(language, "portfolio.riskGrowth");
  return t(language, "portfolio.riskBalanced");
}
